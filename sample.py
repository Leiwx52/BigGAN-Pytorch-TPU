...
def run(config):
    ...
    # Here is where we deal with the ema--load ema weights or load normal
    # weights
    utils.load_weights(
        G if not (
            config['use_ema']) else None,
        None,
        state_dict,
        config['weights_root'],
        experiment_name,
    ...
    if config['accumulate_stats']:
        print(
            'Accumulating standing stats across %d accumulations...' %
            config['num_standing_accumulations'])
        utils.accumulate_standing_stats(G, z_, y_, config['n_classes'],
                                        config['num_standing_accumulations'])

    # Sample a number of images and save them to an NPZ, for use with
    # TF-Inception
    ...
    if config['sample_random']:
        ...
        torchvision.utils.save_image(
            images.float(), '%s/%s/random_samples.jpg' %
            (config['samples_root'], experiment_name), nrow=int(
                G_batch_size**0.5), normalize=True)
    ...
    # Sample truncation curve stuff. This is basically the same as the
    # inception metrics code
        ...
        for var in np.arange(start, end + step, step):
            ...
            if config['accumulate_stats']:
                utils.accumulate_standing_stats(
                    G, z_, y_, config['n_classes'], config['num_standing_accumulations'])
            get_metrics()
