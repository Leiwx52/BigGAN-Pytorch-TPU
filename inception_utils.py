...
# Calculate Inception Score mean + std given softmax'd logits and number
# of splits
def calculate_inception_score(pred, num_splits=10):
    ...
    kl_inception = pred * \
        (torch.log(pred) - torch.log(torch.mean(pred, 0)).unsqueeze(0))
    kl_inception = torch.mean(torch.sum(kl_inception, 1))
...
def accumulate_inception_activations(
        model_sample, net, num_inception_images=50000):
    # get batch size
...
def prepare_inception_metrics(
    ...
    def get_inception_metrics(
        ...
        pool, logits = accumulate_inception_activations(
            model_sample, net, num_inception_images)
        if no_inception:
            IS_mean = 0.0
            IS_std = 0.0
        else:
        ...
        if no_fid:
            ...
            FID = float(
                torch_calculate_frechet_distance(
                    mu,
                    sigma,
                    torch.tensor(data_mu).to(device),
                    torch.tensor(data_sigma).to(device)).cpu().numpy())
