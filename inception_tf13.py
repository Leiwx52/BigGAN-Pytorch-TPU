...
def run(config):
    ...
    def get_inception_score(images, splits=10):
        ...
        with tf.Session() as sess:
            ...
            return np.mean(scores), np.std(
                scores), np.squeeze(np.concatenate(pools, 0))
    # Init inception
    ...
    np.savez(
        '%s/%s/TF_pool.npz' %
        (config['experiment_root'], config['experiment_name']), **{
            'pool_mean': np.mean(
                pool_activations, axis=0), 'pool_var': np.cov(
                pool_activations, rowvar=False)})
    print(
