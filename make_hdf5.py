...
def run(config):
    ...
    for i, (x, y) in enumerate(tqdm(train_loader)):
        # Stick X into the range [0, 255] since it's coming from the train
        # loader
        ...
        if i == 0:
            with h5.File(config['data_root'] + '/ILSVRC%i.hdf5' % config['image_size'], 'w') as f:
                ...
                imgs_dset[...] = x
                labels_dset = f.create_dataset('labels', y.shape, dtype='int64', maxshape=(len(
                    train_loader.dataset),), chunks=(config['chunk_size'],), compression=config['compression'])
