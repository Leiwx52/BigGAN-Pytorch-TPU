...
def power_iteration(W, u_, update=True, eps=1e-12):
    ...
    for i, u in enumerate(u_):
        ...
        with torch.no_grad():
            v = torch.matmul(u, W)
            # Run Gram-Schmidt to subtract components of all other singular
            # vectors
...
class SN(object):
    ...
    # Singular values;
    # note that these buffers are just for logging and are not used in
    # training.
    @property
    def sv(self):
...
def fused_bn(x, mean, var, gain=None, bias=None, eps=1e-5):
    ...
    # return ((x - mean) / ((var + eps) ** 0.5)) * gain + bias # The unfused
    # way.
