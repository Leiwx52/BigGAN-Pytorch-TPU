...
def prepare_parser():
    ...
    parser.add_argument(
        '--shared_dim', type=int, default=0, help='G'
...
def update_config_roots(config):
    if config['base_root']:
        xm.master_print(
            'Pegging all root folders to base root %s' %
            config['base_root'])
        for key in ['weights', 'logs', 'samples']:
            # '%s/%s' % (config['base_root'], key)
            config['%s_root' % key] = os.path.join(config['base_root'], key)
    return config
...
def ortho(model, strength=1e-4, blacklist=[]):
    with torch.no_grad():
        for param in model.parameters():
            # Only apply this to parameters with at least 2 axes, and not in
            # the blacklist
...
def default_ortho(model, strength=1e-4, blacklist=[]):
    with torch.no_grad():
        for param in model.parameters():
            # Only apply this to parameters with at least 2 axes & not in
            # blacklist
...
def save_weights(G, D, state_dict, weights_root, experiment_name,
    ...
    xm.save(state_dict, '%s/%s.pth' %
            (root, join_strings('_', ['state_dict', name_suffix])))

    if G_ema is not None:
        xm.save(G_ema.state_dict(), '%s/%s.pth' %
...
def load_weights(G, D, state_dict, weights_root, experiment_name,
    ...
    if G is not None:
        G.load_state_dict(
            torch.load(
                ...
                (root, join_strings(
                    '_', [
                        'G', name_suffix])), map_location=torch.device('cpu')), strict=strict)
        if load_optim:
            G.optim.load_state_dict(
                torch.load(
                    ...
                    (root, join_strings(
                        '_', [
                            'G_optim', name_suffix])), map_location=torch.device('cpu')))
    if D is not None:
        D.load_state_dict(
            torch.load(
                '%s/%s.pth' %
                (root, join_strings(
                    '_', [
                        'D', name_suffix])), map_location=torch.device('cpu')), strict=strict)
        if load_optim:
            D.optim.load_state_dict(
                torch.load(
                    ...
                    (root, join_strings(
                        '_', [
                            'D_optim', name_suffix])), map_location=torch.device('cpu')))
    # Load state dict
    for item in state_dict:
        state_dict[item] = torch.load(
            ...
            (root, join_strings(
                '_', [
                    'state_dict', name_suffix])), map_location=torch.device('cpu'))[item]
    if G_ema is not None:
        G_ema.load_state_dict(
            torch.load(
                ...
                (root, join_strings(
                    '_', [
                        'G_ema', name_suffix])), map_location=torch.device('cpu')), strict=strict)
...
class MyLogger(object):
    ...
    # Log in plaintext; this is designed for being read in MATLAB(sorry not
    # sorry)
...
def sample_sheet(
    ...
    for i in range(num_classes // classes_per_sheet):
        ...
        image_filename = '%s/%s/%d/samples%d.jpg' % (
            samples_root, experiment_name, folder_number, i)
        torchvision.utils.save_image(out_ims, image_filename,
                                     nrow=samples_per_class, normalize=True)


# Interp function; expects x0 and x1 to be of shape (shape0, 1,
# rest_of_shape..)
    ...
    name = '_'.join(
        [
            item for item in [
                ...
                config['D_ch'],
                'Gd%d' %
                config['G_depth'] if config['G_depth'] > 1 else None,
                'Dd%d' %
                config['D_depth'] if config['D_depth'] > 1 else None,
                'bs%d' %
                config['batch_size'],
                ...
                'nDa%d' %
                config['num_D_accumulations'] if config['num_D_accumulations'] > 1 else None,
                'nGa%d' %
                config['num_G_accumulations'] if config['num_G_accumulations'] > 1 else None,
                'Glr%2.1e' %
                config['G_lr'],
                'Dlr%2.1e' %
                config['D_lr'],
                'GB%3.3f' %
                config['G_B1'] if config['G_B1'] != 0.0 else None,
                'GBB%3.3f' %
                config['G_B2'] if config['G_B2'] != 0.999 else None,
                'DB%3.3f' %
                config['D_B1'] if config['D_B1'] != 0.0 else None,
                'DBB%3.3f' %
                config['D_B2'] if config['D_B2'] != 0.999 else None,
                'Gnl%s' %
                config['G_nl'],
                'Dnl%s' %
                config['D_nl'],
                ...
                config['D_init'],
                'G%s' %
                config['G_param'] if config['G_param'] != 'SN' else None,
                'D%s' %
                config['D_param'] if config['D_param'] != 'SN' else None,
                'Gattn%s' %
                config['G_attn'] if config['G_attn'] != '0' else None,
                'Dattn%s' %
                config['D_attn'] if config['D_attn'] != '0' else None,
                'Gortho%2.1e' %
...
# https://discuss.pytorch.org/t/subclassing-torch-tensor/23754/2
