...
class Generator(nn.Module):
    def __init__(
        ...
        # For some reason applying SN to G's embedding seems to randomly
        # cripple G
        ...
        self.linear = self.which_linear(self.dim_z +
                                        self.shared_dim, self.arch['in_channels'][0] *
                                        (self.bottom_width ** 2))
                ...
                [
                    GBlock(
                        ...
                        upsample=(
                            functools.partial(
                                F.interpolate,
                                scale_factor=2) if self.arch['upsample'][index] and g_index == (
                                self.G_depth -
                                1) else None))] for g_index in range(
                    self.G_depth)]
...
class Discriminator(nn.Module):
    ...
    def __init__(
            ...
            self.blocks += [
                [
                    DBlock(
                        ...
                        preactivation=True,
                        downsample=(
                            nn.AvgPool2d(2) if self.arch['downsample'][index] and d_index == 0 else None)) for d_index in range(
                        self.D_depth)]]
            # If attention on this block, attach it to the end
            if self.arch['attention'][self.arch['resolution'][index]]:
                print(
        ...
        if D_mixed_precision:
            ...
            self.optim = utils.Adam16(
                params=self.parameters(), lr=self.lr, betas=(
                    self.B1, self.B2), weight_decay=0, eps=self.adam_eps)
        else:
    ...
    def forward(self, x, y=None):
        ...
        out = self.linear(h)
        # Get projection of final featureset onto class vectors and add to
        # evidence
